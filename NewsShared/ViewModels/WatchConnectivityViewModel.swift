...
class WatchConnectivityViewModel: NSObject, ObservableObject, WCSessionDelegate {
      ...
      if let title = message["title"] as? String,
         ...
         let url = URL(string: urlString) {
          DispatchQueue.main.async {
              ...
              } else { // Если приложение в неактивном или фоновом состоянии
  ...
  func sendURLToiPhone(article: Article) {
      ...
      print("@@@send URLToiPhone replyHandler=\(replyHandler)")
        DispatchQueue.main.async {
            self.isSending = false
        }
    } errorHandler: { error in
      print("@@@send URLToiPhone Error=\(error.localizedDescription)")
      DispatchQueue.main.async {
          self.isSending = false
      }
    }
